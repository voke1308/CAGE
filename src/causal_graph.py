...
class CausalGraph:
    ...
    def add_event(self, event: dict) -> list:
        with self._lock:
            alerts = []
            t1059_hit = self._check_t1059(event)
            if t1059_hit:
                alerts.append(t1059_hit)
            t1552_hit = self._check_t1552(event)
            if t1552_hit:
                alerts.append(t1552_hit)
            chain_alerts = self._check_chains(event)
            alerts.extend(chain_alerts)
            
            pod_uid = event.get("pod_uid")
            if pod_uid:
                ts = datetime.fromisoformat(event["timestamp"].replace("Z", "+00:00")) if "Z" in event["timestamp"] else datetime.fromisoformat(event["timestamp"])
    ...
    def _check_chains(self, event: dict) -> list:
        ...
        alerts = []
        has_t1059 = any(
            e.get("event_type") == "process_exec" and
            e.get("binary") in ("/bin/bash", "/bin/sh", "/usr/bin/bash", "/usr/bin/sh")
            for _, e in events
        )
        has_t1552 = any(e.get("event_type") == "k8s_secret_access" for _, e in events)
        if has_t1059 and has_t1552:
            alerts.append({
                "severity": "CRITICAL",
                "rule": "T1059→T1552",
                "description": "Lateral movement chain detected",
                "pod_uid": pod_uid,
                "timestamp": event.get("timestamp"),
            })
        return alerts
